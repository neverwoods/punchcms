...
class LiveUser_Auth_Common
{
    ...
    */
     ...
     * @access public
     */
    function login($handle, $passwd, $auth_user_id = false, $accountId = 0)
    {
        // Init value: Is user logged in?
        $this->loggedIn = false;

        // Read user data from database
        $result = $this->readUserData($handle, $passwd, $auth_user_id, $accountId);
        if (!$result) {
            return $result;
        }
     ...
     * @access public
     */
    function readUserData($handle = '', $passwd = '', $auth_user_id = false, $accountId = 0)
    {
        $this->stack->push(LIVEUSER_ERROR_NOT_SUPPORTED, 'exception',
