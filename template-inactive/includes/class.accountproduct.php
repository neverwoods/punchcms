...
/*
 * AccountProduct Class v0.1.1
 * Retrieves product data related to an account from the database.
 */

class AccountProduct extends DBA_AccountProduct {

	public static function getByAccountId($intAccountId) {
		/**
		*  @desc: Get all products that are in a specific account.
		*  @param: intAccountId - The id of the account in question.
		*  @return: Collection of AccountProduct objects .
		*  @type: public static
		*/

		$strSql = sprintf("SELECT * FROM punch_account_product WHERE accountId = '%s'", $intAccountId);
