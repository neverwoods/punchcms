...
class StorageItems extends DBA__Collection {
	...
	public static function getFileListHTML() {
		...
		$intParentId = (int)request("parentId", 0);
		$intAccountId = $_CONF['app']['account']->getId();
		$arrImages = array('jpg', 'jpeg', 'gif', 'png');

		$strSql = sprintf(
			"SELECT * FROM pcms_storage_item
			WHERE parentId = '%s'
			AND typeId IN (%s)
			AND accountId = '%s'
			ORDER BY name",
		...
		foreach ($objElements as $objElement) {
			...
			$blnIsImage = in_array($strExtension, $arrImages);
			$strImageSrc = "/images/ico_attachment.png";
			$arrAttributes = array("width='28'", "height='52'");
			$strClass = " class='attachment'";

			if ($blnIsImage) {
				$strImageSrc = Setting::getValueByName("web_server")
					. Setting::getValueByName("file_folder")
					. $objData->getLocalName();

				$arrAttributes = array("width='100'", "height='100'");
				$strClass = "";
			}
			...
			/*
			$strReturn .= "<li>
				<a href=\"#\" id=\"eid_{$objElement->getId()}\">
					<img width=\"100\" height=\"100\" src=\"{$strImageSrc}\" alt=\"{$objData->getLocalName()}\" />
				</a>
				<span>" . str_replace("&", "&amp;", $objElement->getName()) . "</span>
			</li>";
			*/

			//*** New
			$strMeta = implode(" ", $arrAttributes);
			$strOriginalLabel = str_replace("&", "&amp;", $objElement->getName());
			$strLabel = getShortValue($strOriginalLabel, 14, false, "&hellip;");
			$blnIsShortened = ((strlen($strLabel) - 1) < strlen($strOriginalLabel));
			$strFullLabel = ($blnIsShortened) ? " title='{$strOriginalLabel}'" : "";

			$strReturn .= "<li>
				<a href=\"#\" id=\"eid_{$objElement->getId()}\"{$strClass}>
					<img src=\"{$strImageSrc}\" alt=\"{$objData->getLocalName()}\" {$strMeta}/>
					<span{$strFullLabel}>{$strLabel}</span>
				</a>
			</li>";
		}
		$strReturn .= "</ul>";
