...
class Account extends DBA_Account {
	...
	public function makeBackup($intMax = 5) {
		...
		rename($strZipFile, $_PATHS['backup'] . $this->id . "_" . strftime("%Y%m%d%H%M%S") . ".zip");
		
		//*** Remove old backups.
		$arrFiles = scandir($_PATHS['backup'], 1);
		$intCount = 1;
		foreach ($arrFiles as $strFileName) {
			if (substr($strFileName, 0, strlen($this->id) + 1) == $this->id . "_") {
				if ($intCount > $intMax) {
	...
	public function getBackups() {
		...
		$arrFiles = scandir($_PATHS['backup'], 1);
		foreach ($arrFiles as $strFileName) {
			echo substr($strFileName, 0, strlen($this->id) + 1) . "<br />";
			if (substr($strFileName, 0, strlen($this->id) + 1) == $this->id . "_") {
				array_push($arrReturn, array("file" => $strFileName, "label" => $strFileName));
			}
		}
