...
/**************************************************************************
* PunchCMS Export2Html script v0.1.0
...
function copyr($source, $dest, $fileFilter = array()){
	...
	if (is_file($source)) {
		if (count($fileFilter) == 0 || in_array(extension($source), $fileFilter)) {
			if ($blnZipCreated === TRUE) $objZip->addFile($source, $dest);
			logExport($source);
			return true;
		} else {
			return false;
...
function exportIndexPage($objLanguage) {
	...
	if ($blnZipCreated === TRUE) $objZip->addFromString($strDirname . "/" . $strBasename, fixLinks($strFile));
...
function exportPage($objPageElement, $objLanguage) {
	...
	if ($blnZipCreated === TRUE) $objZip->addFromString($strDirname . "/" . $strBasename . ".html", fixLinks($strFile));
...
function exportSite() {
	...
	$blnZipCreated = $objZip->open($strZipName, ZipArchive::CREATE);
