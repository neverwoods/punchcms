...
CKEDITOR.dialog.add( 'link', function( editor ) {
	...
	var linkTypeChanged = function() {
			var dialog = this.getDialog(),
				partIds = [ 'urlOptions', 'anchorOptions', 'emailOptions', 'elementOptions', 'mediaOptions' ],
			...
			if ( !retval.type ) {
				...
				// elementRegex matches empty strings, so need to check for href as well.
					...
					retval.url.elementurl = urlMatch;
                                ...
                                // mediaRegex matches empty strings, so need to check for href as well.
				else if ( href && ( href.match( mediaRegex ) ) ) {
                                        urlMatch = href;
					retval.type = 'media';
					retval.url = {};
					retval.url.mediaurl = urlMatch;
				}
			...
			elements: [
				...
				children: [
					...
					children: [
						...
						validate: function() {
							...
							return true;
							var func = CKEDITOR.dialog.validate.notEmpty( linkLang.noUrl + "test" );
				...
				children: [
					...
					children: [
						...
						validate: function() {
							...
							if ( dialog.getContentElement( 'info', 'linkType' ) && dialog.getValueOf( 'info', 'linkType' ) != 'element' )
								return true;
				...
				children: [
					...
					filebrowser: {
                                            ...
                                            url: '/inc.filebrowser.php?data=media'
                                        },
					label: commonLang.browseServer
				}
				]
		...
		onOk: function() {
			...
			switch ( data.type || 'url' ) {
				case 'url':
					...
					break;
                                case 'element':
                                        var url = ( data.url && CKEDITOR.tools.trim( data.url.elementurl ) ) || '';
					attributes[ 'data-cke-saved-href' ] = ( url.indexOf( '/' ) === 0 ) ? url : url;
					break;
				case 'media':
                                        var url = ( data.url && CKEDITOR.tools.trim( data.url.mediaurl ) ) || '';
					attributes[ 'data-cke-saved-href' ] = ( url.indexOf( '/' ) === 0 ) ? url : url;
					break;
				case 'anchor':
