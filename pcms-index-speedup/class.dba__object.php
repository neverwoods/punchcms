...
class DBA__Object
{
	...
	public function save($blnSaveModifiedDate = true)
	{
		...
		if (!is_object($DBAConn)) {
			die ("Connection Error in " . self::$object . "::save on line " . __LINE__ . ".<br /><b>Error Details</b>: " . $DBAConn->errorInfo());
		}

		$objResult = $DBAConn->exec($strSql);

		if ($objResult === false) {
			die ("Database Error in " . self::$object . "::save on line " . __LINE__ . ".<br /><b>Error Details</b>: " . $DBAConn->errorInfo() . "<br />Trying to execute: " . $strSql);
		}
	...
	public function delete($accountId = null)
	{
		...
		if ($this->id > 0) {
			...
			if (!is_object($DBAConn)) {
				die ("Connection Error in " . self::$object . "::delete on line " . __LINE__ . ".<br /><b>Error Details</b>: " . $DBAConn->errorInfo());
			}

			$objResult = $DBAConn->exec($strSql);

			if ($objResult === false) {
				die ("Database Error in " . self::$object . "::delete on line " . __LINE__ . ".<br /><b>Error Details</b>: " . $DBAConn->errorInfo() . "<br />Trying to execute: " . $strSql);
			}
	...
	public function duplicate()
	{
		...
		if ($this->id > 0) {
			...
			if (!is_object($DBAConn)) {
				die ("Connection Error in " . self::$object . "::duplicate on line " . __LINE__ . ".<br /><b>Error Details</b>: " . $DBAConn->errorInfo());
			}

			$objResult = $DBAConn->query($strSql);

			if ($objResult === false) {
				die ("Database Error in " . self::$object . "::duplicate on line " . __LINE__ . ".<br /><b>Error Details</b>: " . $DBAConn->errorInfo() . "<br />Trying to execute: " . $strSql);
			}
	...
	public static function selectByPK($varValue, $arrFields = array(), $accountId = null)
	{
		...
		if (isset($strSql)) {
			if (!is_object($DBAConn)) {
				die ("Connection Error in " . self::$object . "::selectByPK on line " . __LINE__ . ".<br /><b>Error Details</b>: " . $DBAConn->errorInfo());
			}

			$objResult = $DBAConn->query($strSql);

			if ($objResult === false) {
				die ("Database Error in " . self::$object . "::selectByPK on line " . __LINE__ . ".<br /><b>Error Details</b>: " . $DBAConn->errorInfo() . "<br />Trying to execute: " . $strSql);
			}
	...
	public static function select($strSql = "")
	{
		...
		if (!is_object($DBAConn)) {
			die ("Connection Error in " . self::$object . "::select on line " . __LINE__ . ".<br /><b>Error Details</b>: " . $DBAConn->errorInfo());
		}
		...
		if ($objResult === false) {
			die ("Database Error in " . self::$object . "::select on line " . __LINE__ . ".<br /><b>Error Details</b>: " . $DBAConn->errorInfo() . "<br />Trying to execute: " . $strSql);
		}
