...
class ContentLanguage extends DBA_ContentLanguage {
	...
	public static function selectByAbbr($strAbbr) {
		...
		$strSql = sprintf("SELECT * FROM " . parent::$table . " WHERE abbr = '%s' AND accountId = '%s' ORDER BY sort", quote_smart($strAbbr), $_CONF['app']['account']->getId());
	...
	public static function hasLanguage($strLanguage) {
		...
		$strSql = sprintf("SELECT * FROM " . self::$table . " WHERE `name` = '%s' AND `accountId` = '%s'", quote_smart(strtolower($strLanguage)), $_CONF['app']['account']->getId());
		$objReturns = self::select($strSql);
